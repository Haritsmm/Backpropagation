...
"""Streamlit application for predicting students' academic potential."""
...
from predictor import FEATURE_COLUMNS, predict_potensi

DATA_PATH = Path("data_siswa_seimbang.csv")
...
def load_data() -> pd.DataFrame:
    ...
    return pd.DataFrame(columns=FEATURE_COLUMNS + ["Potensi"])
...
def main() -> None:
    ...
    st.markdown(
        """
        Silakan isi data Anda di bawah ini. Sistem akan memprediksi potensi akademik
        Anda (Sains, Bahasa, Sosial, Teknologi) dan membandingkan dengan data siswa
        lainnya.
        """
    )
